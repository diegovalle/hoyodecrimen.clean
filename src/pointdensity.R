...
points <- read.csv("clean-data/crime-lat-long-pgj.csv", stringsAsFactors = FALSE) %>%
  filter(date <= "2018-12-31" & date >= "2016-01-01") %>%
...
qmplot(lon, lat, data = SD_density, geom = "blank",  
  ...
  labs(title = "Point density of homicide counts in CDMX with a 1 km radius (2016-2018)") +
  coord_map()
...
breaks = c(as.numeric(as.Date(c("2016-07-01", "2017-07-01", "2018-07-01"))))
qmplot(lon, lat, data = SD_density, geom = "blank",  
