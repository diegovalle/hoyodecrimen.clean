print("Cleaning PGJ-CDMX data and merge with colonias")
...
suppressWarnings(
  col_map <- readOGR(file.path("data/colonias", "colonias_pob.shp"),
                      layer = "colonias_pob",
...
col_map@data$sumPOB1[is.na(col_map@data$sumPOB1)] <- 0
...
ID <- spTransform(ID, proj4string(col_map))
df <- left_join(df,
                cbind(as.data.frame(ID)[, "id", drop = FALSE],
                      sp::over(ID, col_map, returnList = FALSE)[,"sumPOB1"]),
                by = "id"
)
df <- dplyr::rename(df,
                    "population" = "sp::over(ID, col_map, returnList = FALSE)[, \"sumPOB1\"]")
...
cuadrantes <- cuadrantes %>%
  group_by(date, crime, Colonia) %>%
  summarise(count = n(), population = population[1]) %>%
  ungroup()
